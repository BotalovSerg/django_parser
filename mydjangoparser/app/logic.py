...
class ParserMain:
    ...
    def save_data(self, company_name, address, url_map, link, phone_number, email):
    ...
    def get_data(self, query):
        ...
        while len(companies) <= 4:
            scrollable_div = self.browser.find_element(By.XPATH, '//div[@role="main"]/div[2]/div[1]')
            ...
            if soup.select_one('button[aria-label=" Next page "][disabled="true"]'):
                ...
                next_page = self.browser.find_element(By.CSS_SELECTOR, 'button[aria-label=" Next page "]')
...
#     parser.run()
# p = ParserMain()
# p.get_data('Екатеринбург+ресторан')
